...
?>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="view/validAccounts/style.css?version=1">
	...
	<body id="setFont">
		...
		?>
		<h2 id="title">COMPTES PRATICIEN VALIDES</h2>
		...
		<div id="dashboardMenuOptions">
			...
			<?php
					...
					{
						while ($rows = $validPraticienAccounts -> fetch() ) 
						{
			...
			?>
							<tr> 
							    ...
							    <td><?= base64_decode($rows['password']) ?></td> 
							    ...
							    <td><a href="index.php?action=rejectThisPraticienValidAccount&id=<?= $rows['id'] ?>">Refuser</a></td>
							    <!--<td><a href="index.php?action=deleteThisPraticienValidAccount&id=<?= $rows['id'] ?>">Supprimer</a></td>-->
							</tr>
			<?php
						}
				 	} 
		...
		<h2 id="title">COMPTES CLIENTS VALIDES</h2>
		...
		<div id="dashboardMenuOptions">
			...
			<?php
					...
					{
						while ($rows2 = $validClientAccounts -> fetch() ) 
						{
			...
			?>
							<tr> 
							    ...
							    <td><a href="index.php?action=rejectThisClientValidAccount&id=<?= $rows2['id'] ?>">Refuser</a></td>
							   <!-- <td><a href="index.php?action=deleteThisClientValidAccount&id=<?= $rows2['id'] ?>">Supprimer</a></td>-->
							</tr>
			<?php
						}
				 	} 
